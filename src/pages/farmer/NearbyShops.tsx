...
import { useDiseaseNotification } from "@/services/notificationService";
...
const NearbyShops = () => {
  ...
  const { notifyMerchants } = useDiseaseNotification();
  
  // Check if there's a disease detection result from navigation state
  const diseaseDetectionResult = location.state?.detectionResult || 
    (location.state?.aiAnalysis ? {
      diseaseName: location.state.aiAnalysis.disease_name || 'Unknown Disease',
      diseaseDescription: location.state.aiAnalysis.description || 'Analysis completed',
      recommendedTreatment: location.state.aiAnalysis.treatments?.[0]?.name || 'Consult local agricultural expert',
    } : null);
  ...
  const handleNotifyNearbyShops = async () => {
    if (!diseaseDetectionResult || !userLocation) {
      toast.error('Disease detection result or location not available');
      return;
    }
    ...
    try {
      await notifyMerchants(
        diseaseDetectionResult.diseaseName,
        diseaseDetectionResult.diseaseDescription,
        diseaseDetectionResult.recommendedTreatment,
        userLocation
      );
    } catch (error) {
            ...
            <CardContent className="p-6">
              <div className="flex items-start justify-between">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-lg mb-1">Disease Detection Result Available</h3>
                    <p className="text-muted-foreground text-sm mb-2">
                      <strong>{diseaseDetectionResult.diseaseName}</strong> detected. 
                      Recommended treatment: {diseaseDetectionResult.recommendedTreatment}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Notify nearby agricultural shops to check availability of required treatments.
                    </p>
                  </div>
                </div>
                <Button
                  onClick={handleNotifyNearbyShops}
                  disabled={notifying}
                  className="glass-subtle border border-primary/30 hover:border-primary/50 btn-ripple"
                >
                  {notifying ? (
                    <>
