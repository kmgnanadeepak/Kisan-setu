...
const DiseaseResult = () => {
 ...
 const downloadPrescription = (content) => {
  ...
  doc.setFontSize(16);
  doc.text("KisanSetu Disease Prescription", pageWidth / 2, y, { align: "center" });
  y += 12;
  ...
  lines.forEach((line) => {
    ...
    if (
      ...
      line.startsWith("Disease Detected") ||
      line.startsWith("Severity") ||
      line.startsWith("Weather Conditions") ||
      line.startsWith("Land Area") ||
      line.startsWith("Recommended Treatments") ||
      line.startsWith("Application Guidelines") ||
  ...
  doc.save(`Prescription_${today}.pdf`);
};
...
- ${t.name}
  ...
  Estimated Savings: ₹${t.estimatedProfit.toFixed(2)}
`).join('')}

Application Guidelines:
${disease.applicationGuide.map((guide, index) => `
${index + 1}. ${guide.step}
   Timing: ${guide.timing}
`).join('')}

Important Notes:
- Apply treatment in early morning for best absorption
- Avoid application if rain expected within 4 hours
- Follow safety precautions and wear protective equipment
- Store remaining product in cool, dry place

Generated by KisanSetu - The Farmer's Bridge
Contact: support@kisansetu.com | Emergency: 1800-XXX-XXXX
`.trim();

  downloadPrescription(content);
};
                    ...
                    >
                      ...
                      <div className="flex items-center gap-4 mt-4">
                        ...
                        <Button variant="outline" size="sm" className="flex-1" onClick={() => {
                          // Navigate to nearby shops with disease detection result
                          navigate('/farmer/nearby-shops', {
                            state: {
                              detectionResult: {
                                diseaseName: disease.name,
                                diseaseDescription: disease.description,
                                recommendedTreatment: disease.treatments[0]?.name || 'Consult local agricultural expert',
                              },
